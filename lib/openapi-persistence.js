...
import getDemoUserAndProject from './demo-context.js';
...
export async function persistOpenApiSpecForDemo({ rawSpec, format, existingSpecId } = {}, client = prisma) {
  const { user, project } = await getDemoUserAndProject(client);
  const specFormat = format || detectSpecFormat(rawSpec);

  if (existingSpecId) {
    const spec = await client.openApiSpec.findUnique({ where: { id: Number(existingSpecId) } });
...
export async function upsertApiConnectionForDemo({ baseUrl = '', auth } = {}, client = prisma) {
  const { user, project } = await getDemoUserAndProject(client);
  ...
  const existing = await client.apiConnection.findFirst({ where: { projectId: project.id } });
  if (existing) {
...
export async function replaceToolMappingsForDemo({
  operations = [],
  openApiSpecId = null,
  ...
  if (openApiSpecId) {
    await client.toolMapping.deleteMany({ where: { projectId: project.id, openApiSpecId } });
  } else {
    await client.toolMapping.deleteMany({ where: { projectId: project.id } });
  }
